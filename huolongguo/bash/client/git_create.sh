...
# using custom scripts (MySQL import / configuration)
#
post_installation() {
	if [ -z "$POST_INSTALL_FOLDER" -o ! -d "$POST_INSTALL_FOLDER" ]; then
		echo "Post installation folder not defined or missing"
		echo "No post install operation performed"
		echo
	else
		for script in $(ls $POST_INSTALL_FOLDER)
		do
			if [ -x "$script" ]; then
				$POST_INSTALLATION_FOLDER/$script
			fi
		done
	fi
}
