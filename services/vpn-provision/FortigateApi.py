...
class Fortigate:
    ...
    def GetGroup(self, name=''):
        req = self.ApiGet('cmdb/user/group/' + name)
        return req.text
    ...
    def SetUser2Group(self, group_name, members):
        '''set members to specified group'''
        # unicode_member= json.loads(members)['results'][0]['member']
        unicode_members = members
        str_list = []
        tmp_dict = {}
        for each_dict_member in unicode_members:
            tmp_dict['q_origin_key'] = str(each_dict_member['q_origin_key'])
            tmp_dict['name'] = str(each_dict_member['name'])
            str_list.append(tmp_dict.copy())
        ...
        return self.ApiSet('/cmdb/user/group/%s/' % group_name, payload)

    def AddGroupLocal(self, group_name, group_type):
        ...
        return self.ApiAdd('/cmdb/user/group/', payload)
    ...
    def GetFwAddressGroup(self, name=''):
        ...
        Return the json object
        '''
        req = self.ApiGet('cmdb/firewall/addr/' + name)
        return req.text
    ...
    def DelAllFwPolicy(self):
        ...
        for y in range(0,len(data['results'])):
            ...
            print('del fw policy id:{},{}'.format(policy_id,return_code))
            if return_code != 200: return return_code
        return 200
        ...
        for y in range(0,len(data['results'])):
            ...
            for x in range(0,len(objects)):
                ...
                if (type(req_res) is list):
                    ...
                    # print 'object is different:', objects[x][0], ':', objects[x][1] ,'to', req_res
                    print('object is different:{}:{} to {}'.format(objects[x][0],objects[x][1],req_res))
                    identical = False
                    break
            if identical: 
    ...
    def DelAllFwServiceGroup(self):
        ...
        for y in range(0,len(data['results'])):
            ...
            print('del fw service group:{},({})'.format(service_group_name,return_code))
            if return_code != 200: return return_code
        return 200
    #
    ...
    def DelAllVPNipsec(self):
        ...
        for y in range(0,len(data['results'])):
            ...
            print('del vpn:{},res:{}'.format(vpn_name,return_code))
            if return_code != 200: return return_code
        return 200 
